...
# Commented out the following because sslscan is being called from Docker container.
...
# Commented out the following because sslscan is being called from Docker container.
#SLSCANVERSION=$(sslscan --version | grep version | \
#sed "s/^.*sslscan/sslscan/")
SLSCANVERSION=$(docker run --rm=true --interactive --tty jyeary/docker-sslscan:latest --version)

echo "    openssl version: $OPENSSLVERSION"
echo "    sslscan version: $SSLSCANVERSION"
echo ------------------------------------------------------
echo "    Date: $DATE" 
echo ------------------------------------------------------
echo
...
TARGET=$HOST\_$PORT
DIRECTORY=results/TLSSLed\_$VERSION\_$TARGET\_$DATE
...
docker run --rm=true --interactive --tty jyeary/docker-sslscan:latest \
$HOST:$PORT > $DIRECTORY/$SSLSCANLOGFILE \
